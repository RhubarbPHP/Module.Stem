...
use Rhubarb\Stem\Tests\unit\Fixtures\AccountSchema;
...
class ModelTest extends ModelUnitTestCase
{
    ...
    public function testFindFirst()
    {
        ...
        $account2 = new AccountSchema();
        $account2->AccountID = 'test2';
        $account2->AccountName = 'SAME';
        $account2->save();

        $first = AccountSchema::findFirst();
        $last = AccountSchema::findLast();

        $this->assertEquals('test1', $first->AccountID);
        $this->assertEquals('test2', $last->AccountID);

        $first = AccountSchema::findFirst(new Equals("AccountName", "SAME"));
        $last = AccountSchema::findLast(new Equals("AccountName", "SAME"));
